...
export default function KitchenMenuManager() {
  ...
  async function load() {
    ...
    try {
      // No ?all=1 => API returns ONLY active items
      const r = await fetch('/api/menu', { cache:'no-store', credentials:'include' });
      const j = await r.json();
      if (!r.ok) throw new Error(j?.error || 'Load failed');
      const arr = Array.isArray(j.items) ? j.items : [];
      setItems(arr.map((x:any)=>({
        id: String(x.id), name: String(x.name), category: String(x.category ?? 'hot'), active: Boolean(x.active ?? true),
      })));
    } catch (e:any) { setMsg({ kind:'error', text: e?.message || 'Load failed' }); }
  ...
  async function addItem(name: string, category: string) {
    ...
    try {
      ...
      setMsg({ kind:'ok', text:'Item added' }); void load();
    } catch(e:any){ setMsg({ kind:'error', text:e?.message || 'Add failed' }); }
  }
  ...
  async function archive(id: string) {
    ...
    try {
      ...
      void load(); // item disappears
    } catch(e:any){ setMsg({ kind:'error', text:e?.message || 'Archive failed' }); }
  }

  return (
    <div className="p-4 space-y-4">
      <h1 className="text-lg font-semibold">Kitchen › Menu items (active)</h1>
      ...
      {loading ? <div className="text-sm text-gray-600">Loading…</div> : (
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
          {CATEGORIES.map(cat => {
            const list = items.filter(i=>i.category===cat.key && i.active);
            return (
              <div key={cat.key} className="rounded-xl border p-3 shadow-sm">
                <div className="font-medium mb-2">{cat.label}</div>
                {list.length===0 ? <div className="text-sm text-gray-500">No items</div> : (
                  <ul className="space-y-1">
                    {list.map(i=>(
                      <li key={i.id} className="flex items-center justify-between rounded px-2 py-1 hover:bg-gray-50">
                        <span className="truncate">{i.name}</span>
                        <button className="text-xs rounded border px-2 py-1 hover:bg-gray-50"
                                onClick={()=>void archive(i.id)}
                                title="Archive (hide permanently)">
                          Archive
                        </button>
                      </li>
                    ))}
...
function AddForm({ onAdd }: { onAdd:(name:string, category:string)=>void }) {
  ...
  return (
    <div className="flex flex-wrap items-center gap-2">
      <input value={name} onChange={e=>setName(e.target.value)} placeholder="New dish name…"
      ...
      <button disabled={disabled}
              className={`rounded border px-3 py-2 hover:bg-gray-50 ${disabled?'opacity-50 cursor-not-allowed':''}`}
